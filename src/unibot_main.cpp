...
{
    output[0] = state[5]; // theta (pitch of the rod)
    //printf("pitch of rod = %f\n", output[0]);
    output[1] = output[0] + state[7]; // psi (wheel angle) = pitch of rod + rodwheel angle
    //printf("wheel angle = %f\n", output[1]);
    output[2] = state[13]; // theta_dot
    output[3] = output[2] + state[15]; // psi_dot
}
...
int main(int argc, char* argv[])
{
    ...
    auto unibot_mip_converter = builder.AddSystem(std::make_unique<StateConverter<double>>(unibot_to_mip_func, 16, 4));
    ...
    builder.Connect(plant.get_state_output_port(), unibot_mip_converter->get_input_port());
